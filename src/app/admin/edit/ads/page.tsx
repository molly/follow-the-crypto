...
import { fetchAds, fetchConstant } from "@/app/actions/fetch";
import { db } from "@/app/lib/db";
import { Ad, AdConstant, AdGroup } from "@/app/types/Ads";
import { isError } from "@/app/utils/errors";
import { doc, setDoc } from "firebase/firestore";
import { Fragment, useEffect, useState } from "react";
import styles from "../../admin.module.css";

async function saveAdConstants(adsConstants: Record<string, AdConstant>) {
...
export default function AdsEditor() {
  ...
  const [adConstants, setAdConstants] = useState<Record<string, AdConstant>>(
    {},
  );
  ...
  useEffect(() => {
    (async () => {
      const [adsData, adsConstantData] = await Promise.all([
        fetchAds(),
        fetchConstant<Record<string, AdConstant>>("ads"),
      ]);
      if (isError(adsData)) {
          ...
          <div className={styles.adRegion}>
            <div>
              ...
              <input
                ...
                id={`race-${ad.ad_id}`}
                value={
                  ad.ad_id in adConstants ? adConstants[ad.ad_id]["race"] : ""
                }
                onChange={(e) => {
                  setAdConstants({
                    ...adConstants,
